...
class Customer extends ApiResource {
  constructor(appId?: string, secretKey?: string) {
    super("customer", appId, secretKey);
  }
  ...
  async createCheckoutSession(customer_uid: string, product_uid: string, version?: number) {
    const path = `/api/v${version}/${this._objectName}/${customer_uid}/checkout-session/${product_uid}`;
    return this._requestor.request('POST', path);
  }

  async createSandboxSubscription(customer_uid: string, product_uid: string, data: any, version?: number) {
  ...
  async createPaymentLink(customer_uid: string, product_id: string, data: any, version?: number) {
